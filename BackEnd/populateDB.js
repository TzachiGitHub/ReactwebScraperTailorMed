...
module.exports = {addProgramsInfo}
...
async function addProgramsInfo(){
    ...
    for(let i = 1; i < 6; i++){
        ...
        // link == programName[1]
        let programName = await page.$eval(".section > div > .subsection > .funds > li:nth-child(" + i + ") > a", (el) => [el.textContent, el.href]);
    ...
    initialPrograms.forEach((program)=>{
        Programs.findOneAndUpdate( program, program, { upsert: true }, function(err,doc) {
            // console.log( doc );
        });
    })
